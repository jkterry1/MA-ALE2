...
def sample_nfsp_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    reservoir_buffer_size = trial.suggest_categorical("reservoir_buffer_size", [int(1e5), int(5e5), int(1e6), int(2e6)])
    hyperparams = {
...
def sample_nfsp_rainbow_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    nfsp_buffer_size = trial.suggest_categorical("nfsp_buffer_size", [int(1e4), int(5e4), int(1e5)])

    hyperparams = sample_rainbow_params(trial)
    hyperparams.update(sample_nfsp_params(trial))
    hyperparams["replay_buffer_size"] = nfsp_buffer_size
    return hyperparams
